...
| Dashboard | Jobs | Skills |
| :---: | :---: | :---: |
| ![Dashboard](docs/dashboard.png) | ![Jobs](docs/jobs.png) | ![Skills](docs/skills.png) |
...
```bash
git clone https://github.com/JacobStew88/CS467_JobTracker.git
cd CS467_JobTracker
```
...
```
PORT=3000
DATABASE_URL=mysql://user:password@localhost:3306/jobtracker
...
```bash
cd tests
newman run job_tracker_full_test_suite_v5.postman_collection.json \
  -e job_tracker_local.postman_environment.json
```
...
```json
{ "username": "dev_user", "password": "StrongPassword123!" }
```
